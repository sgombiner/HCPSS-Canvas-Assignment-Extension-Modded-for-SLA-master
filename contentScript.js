...
BASE_URL = "https://scienceleadership.instructure.com/api/v1";
...
var important_dates_sidebar = "important-dates-sidebar";

/* Wait for the side-bar to load before attempting to modify it. */
function start () {

    // Start by populating the unsubmitted assignments list
    getAssignmentsSidebar().then(assignmentsElement => {

            // If the important dates sidebar element is present, insert
            // the assignments sidebar element into it
            let el = document.getElementById(important_dates_sidebar);

            if (el) {
                el.appendChild(assignmentsElement);
            }


            // Watch the body element for changes
                ...
                for(const mutation of mutationsList) {
                    if (mutation.type === 'childList') {
                        for (const node of mutation.addedNodes) {
                            // This label is used when in small width mode and the sidebar is
                            // moved to a tray
                            if (node.ariaLabel == "Important Dates Tray") {
                                node.parentElement.appendChild(assignmentsElement);
                            }
                            // If there's an important-dates-sidebar element and no
                            // tray label, we're not in small width mode
                            else if (node.id == important_dates_sidebar &&
                                !document.querySelector('[aria-label="Important Dates Tray"]'))
                            {
                                node.appendChild(assignmentsElement);
                            }
                        }
                    }
                }
        ...
        // Worry about the failure case later
        null
    );
}
...
async function getAssignmentsSidebar() {
    let assignments_container = document.createElement("div");
    assignments_container.id = "canvas-assignment-extension-container";
    ...
    if (observees.length > 0)
    {
        ...
        {
            addStudentAssignments(observees[i].id, `${observees[i].name}'s`, assignments_container);
        }
    }
    else
    {
        // We're a student
        addStudentAssignments("self", "My", assignments_container);
    }
